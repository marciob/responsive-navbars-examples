...
const Navbar = () => {
  ...
  const [color, setColor] = useState("transparent"); // it's used to change menu color in mobile devices when scrolling
  ...
  //it sets menu colors for when scrolling
  useEffect(() => {
    const changeColor = () => {
      if (window.scrollY >= 90) {
        //when scrolling it changes to those colors
        setColor("blue");
        setTextColor("black");
      } else {
        //when in initial mode it starts with those colors
